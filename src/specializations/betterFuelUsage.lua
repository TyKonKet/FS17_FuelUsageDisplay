...
function BetterFuelUsage:drawRightMeter(value)
    ...
    for i = math.pi + step, 2 * math.pi, step do
        ...
        overlay:setPosition(
            g_currentMission.vehicleSpeedBg.x + g_currentMission.vehicleSpeedBg.width * 0.5 + posX - g_currentMission.speedMeterIconOverlay.width * 0.5,
            g_currentMission.vehicleSpeedBg.y + g_currentMission.speedMeterCenterOffsetY + posY - g_currentMission.speedMeterIconOverlay.height * 0.5
        )
        overlay:render()
...
function BetterFuelUsage:debugDraw()
    if BetterFuelUsage.debug then
        ...
        if dbgObj.motor ~= nil then
            table.insert(
                self.debugDrawTexts,
                string.format("Motor Rpm --> min:%.0f cur:%.0f max:%.0f factor:%.2f", dbgObj.motor:getMinRpm(), dbgObj.motor:getEqualizedMotorRpm(), dbgObj.motor:getMaxRpm(), (dbgObj.motor:getEqualizedMotorRpm() - dbgObj.motor:getMinRpm()) / (dbgObj.motor:getMaxRpm() - dbgObj.motor:getMinRpm()))
            )
        end
